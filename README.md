# 🧪 AI/ML Projects Repository

This repository contains the complete collection of machine learning projects I’ve built as part of my AI/ML journey.

Each project is designed to reinforce theoretical understanding with practical implementation using real-world datasets.

---

## 🔍 Projects

| Project Title              | Goal                                    | Link                                            |
| -------------------------- | ------------------------------------------ | ----------------------------------------------- |
| Linear Regression From Sratch | Building the model from the scratch by own to get more deeper knowledge| [View Project](./Laptop_Price_Predictor) |
| Laptop Price Predictor | Predict the price of a laptop based on features like Ram, Processor, Storage, GPU, Sreen Size, etc.| [View Project](./Laptop_Price_Predictor) |
| Diabaties Predictor | Predict Wether the perosn have Diabetes or not using features like Glucose, BloodPressure, SkinThickness, Age, etc.| [View Project](./Diabetes_Predictor) |
| Titanic Survival Prediction | Predict Wether the perosn have Survived or not mainly predicting on Gender and Class| [View Project](./Diabetes_Predictor) |

---

## 🎯 Objective

To develop a portfolio of high-quality, end-to-end ML projects that demonstrate:

- Clear understanding of core ML concepts
- Ability to work with real-world datasets
- Proficiency in building and evaluating ML models
- Readiness for internships, hackathons, and research opportunities

> 🔍 Each project includes detailed documentation, code, and results for transparency and learning purposes.

---
